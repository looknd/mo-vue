<template>
	<ul class="mo-paging">
		...
		v-if="config.prev"
		@click="prev" v-html="config.prev" :title="index === 1 ? '' : '上一页'"></li>
		...
		v-if="config.next" @click="next" v-html="config.next" :title="index === pages ? '' : '下一页'"></li>
	...
	export default {
		...
		data () {
			const config = extend({}, {
				first: false,
				last : false,
				prev : '<i class="mo-icon__arrow_left"></i>',
				next : '<i class="mo-icon__arrow_right"></i>',
				more : '...',
				pager : true
			}, this.options)
